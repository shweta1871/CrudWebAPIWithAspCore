        ...
        public static void Initialize(IApplicationBuilder app)
        {
            using (var serviceScope = app.ApplicationServices.GetService<IServiceScopeFactory>().CreateScope())
            {
                ...
                if (context.Ailments != null && context.Ailments.Any())
                    return;   // DB has already been seeded. It means it will check the table & data with 'ailment' in db. If already exists then will return. 
                //Otherwise call getAilments and save in db.
